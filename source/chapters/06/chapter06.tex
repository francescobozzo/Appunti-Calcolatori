...
Esistono però alcune eccezioni: ad esempio \register{gcc}, l'assembler di Unix, usa i commenti come il C e l'operando di destinazione delle operazioni va in fondo.
...
L5:
	j $ra                  # torna al programma chiamante
\end{minted}
Alcuni compilatori sono in grado di ottimizzare automaticamente il programma in questo modo così da renderlo molto più veloce e leggero, il \register{gcc} è in grado di aggiustare, ove possibile, questo tipo di procedure con una complessità pari a \emph{O(2)}.
...
Nessun compilatore avrebbe mai lavorato in questo modo. Abbiamo presentato la procedura così affinché fosse didatticamente più efficace, adesso invitiamo il lettore a dare un'occhiata a quella che potrebbe essere la traduzione di un vero compilatore, in particolare \register{gcc}:
\begin{minted}[linenos, tabsize=4]{asm}
COPIA_STRINGA:
	lb $v0, 0($a1)
	sb $v0, 0($a0)
