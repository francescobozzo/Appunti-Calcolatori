...
Esistono due modalità per richiamare una funzione attraverso l'istruzione \mintinline{asm}{call}:
\begin{itemize}
	\item \mintinline{asm}{call address}: invoca la subroutine presente all'indirizzo indicato;
	\item \mintinline{asm}{call *register}: invoca la subroutine presente all'indirizzo contenuto nel registro indicato.
\end{itemize}
Una caratteristica di \emph{Intel} è quella di salvare automaticamente il \emph{link register} nello stack quando una funzione viene richiamata, facilitando di molto la scrittura di funzioni ricorsive. In maniera analoga, l'istruzione \mintinline{asm}{ret}, che viene utilizzata per terminare una funzione, preleva automaticamente dallo stack l'indirizzo da utilizzare nell'instruction pointer.
...
Un esempio in cui\texttt{ <size> }è obbligatorio: \mintinline{asm}{notl 200(%rbp)}.
...
\begin{itemize}
	...
	\item \opcode{push} e \opcode{pop} per leggere e salvare dati sullo stack, senza dover modificare lo stack pointer come avviene in \emph{MIPS};
...
Seppur questa istruzione sia stata implementata per calcolare indirizzi (con indirizzamento indiretto) senza effettuare accessi, principalmente viene utilizzata per un secondo scopo. Essa infatti risulta molto utile per effettuare una somma di due registri, salvandone il risultato in un terzo (anche con eventuali shift). Ricordiamo che, mentre in \emph{MIPS} lavorare con tre registri è la normalità, in \emph{Intel} per ciascuna istruzione possono esserne specificati al massimo due.
...
Come avete potuto notare dalla lista delle istruzioni comuni in \emph{Intel}, sono presenti le istruzioni \mintinline{asm}{inc} e \mintinline{asm}{dec}, che rispettivamente incrementano e decrementano di una unità il valore del registro specificato. Ma per quale motivo non si utilizza la semplice istruzione \mintinline{asm}{add $1, register}?

In \emph{Intel} si è visto che le istruzioni possono essere codificate con una sequenza di bit di lunghezza variabile. Utilizzare la funzione di incremento piuttosto che quella di somma significa non dover memorizzare un valore immediato, con il conseguente risparmio di 16 bit.
