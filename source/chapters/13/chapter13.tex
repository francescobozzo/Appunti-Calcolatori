...
Qualora il numero di blocchi in cache fosse potenza di due sarà sufficiente usare i suoi bit meno significativi dell'indirizzo in memoria. Quanti bit meno significativi bisogna prendere, di preciso? La risposta è in questa formula:
\begin{equation*}
...
Se gli accessi in lettura avvengono con la logica precedente, quelli in scrittura sono più delicati e potrebbero causare problemi di consistenza, ossia potrei avere situazioni in cui si genera un disallineamento tra i dati presenti in memoria e quelli presenti nella memoria principale.
\begin{itemize}
  \item Un sistema che possiamo adoperare per arginare questo problema è il cosiddetto \emph{write-through}, che consiste nel fatto che ogni scrittura viene effettuata direttamente in memoria principale, a prescindere dal fatto che ci sia o meno una miss. In questo modo avrò cache e memoria sempre aggiornati, ma ben si capisce che si tratta di un'operazione costosa; un buona aggiunta potrebbe essere un buffer di scrittura, una sorta di "coda" dove tutte le scritture vengono messe in attesa, che ci permette quindi di accorpare il maggior numero di operazioni possibile e mantenere cache e memoria sempre allineate.
...
Le cache a mappatura diretta sono piuttosto semplici da realizzare, tuttavia presentano un problema: se ho spesso bisogno di locazioni di memoria che si mappano sullo stesso blocco, ho cache miss in continuazione.
Proprio per gestire queste situazioni è nata l'impementazione della cahce completamente associativa, ovvero una cache in cui posso mappare qualsiasi blocco di memoria in qualsiasi blocco di cache.

\subsection{Cache completamente associativa}
Una cahche completamente associativa è una cache in cui ogni blocco della memoria può essere mappato in un qualsiasi blocco della cache.
...
Per questo motivo ho bisogno di $n$ comparatori (uno per ogni blocco di cache) che operino in parallelo; di conseguenza la complessità di realizzazione cresce proporzionalmente al numero di blocchi ed inoltre il costo HW diventa eccessivamente proibitivo appena si aumenta di poco la dimensione della memoria.

\subsection{Cache set-associativa}
Le cache set-associative sono un via di mezzo tra le due che abbiamo visto fin ora.
In sostanza suddividiamo la cache in $n$ blocchi di memoria, ognuno dei quali viene in seguito suddiviso come se fosse una cache a sè stante; fatta questa suddivisione quello che si ottiene è una sorta di scacchiera in cui i blccchi possono essere visti in raggruppamenti di "linee" (righe) e di "vie" (colonne); Alla fine della fiera quello che otteniamo è una combinazione delle due idee: associamo ciascun blocco della memoria periferica a una certa linea e quindi a uno degli $n$ blocchi di quella linea su cui possiamo mappare il blocco di memoria.
...
Il grande vantaggio è che abbiamo una memoria non troppo costosa, dato che la cache associativa viene implementata su blocchi di dimensioni ridotte, tuttavia si mantiene anche, in parte, il vantaggio della strategia associativa con conseguente abbassamento del \emph{miss rate}.
...
\begin{equation*}
	(\textrm{indirizzo blocco}) mod (\textrm{numero blocchi della cache})
\end{equation*}

In maniera simile in una cache set-associativa un blocco di memoria viene mappato in una \emph{linea} data da:
\begin{equation*}
	(\textrm{indirizzo blocco}) mod (\textrm{numero linee della cache})
\end{equation*}
...
Gli ultimi bit rimanenti vengono utilizzati come tag del blocco; si noti che il tag viene confrontato con tutti e quattro i tag che ritornano dalle 4 vie.
...
\begin{itemize}
	...
	\item LeastRecentlyUsed, un metodo più sofisticato che però richiede una serie di bit in più per caclolare quando è avvenutoo l'ultimo accesso.
\end{itemize}
...
\begin{wrapfigure}{l}{0.55\textwidth}
	\centering
	\includegraphics[width=\linewidth,keepaspectratio]{grafici}
