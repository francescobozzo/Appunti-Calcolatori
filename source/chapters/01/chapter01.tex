...
\begin{itemize}
	\item \emph{algoritmi}: determinano il numero di istruzioni di alto livello e di operazioni di \acrfull{io};
	...
	\item \emph{sistema di \acrshort{io}} (\emph{hardware e sistema operativo}): determina quanto velocemente possono essere eseguite le istruzioni.
\end{itemize}
...
\begin{itemize}
	\item \emph{volatile}: è costituita da vari banchi di (tipicamente) \(8\) chip di \acrshort{ram} dinamica. Il tipo più diffuso è appunto la \acrshort{dram};
...
\begin{itemize}
	...
	\item \emph{control unit}: impartisce ordini al datapath, alla memoria e alle componenti \acrshort{io}, sulla base di quanto stabilito dal programma.
\end{itemize}
...
\begin{itemize}
	\item gestione delle operazioni di \acrshort{io};
...
\subsection{Periferiche di \acrshort{io}}
