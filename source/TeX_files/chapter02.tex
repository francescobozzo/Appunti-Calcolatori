...
\subparagraph{Regole di conversione tra basi}
% SI DECIDE DI SALTARE CONVERSIONE TRA BASI E ALTRE BASI UTILI DELIBERATAMENTE %
...
\subparagraph*{Somma e sottrazione} Le operazioni somma e sottrazione funzionano allo stesso modo del sistema utilizzato nella numerazione decimale con il riporto. Si guardino gli esempi qui sotto riportati, già di per sè esplicativi del processo:\\
\begin{figure}[H]
	\centering
...
\subparagraph*{Moltiplicazione} La moltiplicazione in base binaria si può semplificare con il metodo dello shifting, che è più facile illustrare con un esempio:  \begin{figure}[H]
	\centering
...
\paragraph*{Gli interi} Fin ora abbiamo visto come si codificano solo i numeri naturali ma esistono metodi di codifica che permettono di rappresentare anche i numeri negativi (quindi l’insieme degli interi); le tecniche più comuni sono: modulo e segno, complemento a 1 (\textit{CA1}) e complemento a 2 (\textit{CA2}).

\subparagraph*{Codifica con Modulo e Segno:} Idea semplice: si usano $k-1$ bit per rappresentare il valore assoluto (modulo) del numero  ed un bit per codificare il segno (0=positivo, 1=negativo). In questo modo con k bit si possono codificare valori tra $-2k^{-1}+1 \text{ e} +2k^{-1}-1$.\\\\
\textbf{N.B.:} esistono due codifiche per lo $0$, $-0$  e $+0$, e questo è un spreco.
...
\end{itemize}
\subparagraph*{Codifica in Complemento a 2:} Per tradurre un numero da intero con segno a complemento a 2 (\textit{CA2}) basta invertire tutti i bit tranne quello di sego e poi aggiungere 1. Viceversa per convertire da \textit{CA2} a binario con segno si sottrae 1 e poi si invertono tutti i bit tranne quello di segno. \\
Ancora una volta il bit più significativo indica il segno, ma in questo caso la codifica dello 0 è unica, quindi con \textit{k} bit si possono rappresentare i numeri da $2k^{-1}$ a $2k^{-1}-1$. Anche la somma è più semplice ma questo lo vedremo in futuro.
