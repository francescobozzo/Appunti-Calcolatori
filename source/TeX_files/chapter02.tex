...
\end{itemize}
Nel primo di questi due casi i numeri si dicono \textit{denormalizzati}, essi rappresentano tutti i numeri compresi tra 0 e 1, (0 escluso poichè è un carattere speciale); nel secondo caso i numeri si dicono \textit{normalizzati}.\\
Per tradurre in decimale un numero in virgola mobile si deve quindi per prima cosa determinare il segno, poi si calcola l'esponente effettivo dei calcoli ($exp=E-127$) e da qui si procede in due modi: se $exp\neq-127$ si parla di numero \textit{normalizzato} e quindi si aggiunge un bit 1 alla mantissa in modo da ottenere il numero 1,M e si sposta la virgola di $exp$ posizioni e per completare il tutto si traduce il numero binario così ottenuto in decimale.


se $exp=-127$ si parla di numero denormalizzato e quindi 
